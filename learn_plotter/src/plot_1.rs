...
// backendに対してplot
fn plot_q_func(backend: CairoBackend, fun_for_plot: fn(f32) -> f32) -> Result<(), Box<dyn error::Error>> {
        let root = backend.into_drawing_area();
        root.fill(&WHITE)?;

        // 範囲を設定
        const TWO_PI: f32 = std::f32::consts::PI;
        let range = Range::new(-TWO_PI, TWO_PI, 0.01);
        let plot_range = PlotRange::new(-TWO_PI, TWO_PI, -2.0, 2.0);
...
fn build_ui(app: &gtk::Application) {
    ...
    drawing_area.connect_draw(move |widget, cr| {
        ...
        plot_q_func(backend, sq_sin).unwrap();
        Inhibit(false)
    });
